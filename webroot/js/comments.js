...
$(function() {
  ...
  $(document)
    .on('blur', 'input.gravatar', Comments.gravatarBlurEvent)
    .on('click', '.remove-comment', Comments.confirmRemoval);
...
var Comments = {
  ...
  commentFieldBlurEvent : function() {
    ...
    window._hideTimer = setInterval(function() {
      if( jqTextarea.val() == '') {
        jqTextarea.closest('.comment-form:not(.never-hide)').find('.commentator-details').hide('fast');
      }
    }, 300);
...
function ensureFormVisibility(jqElement) {
    var viewportWidth = jQuery(window).width(),
        viewportHeight = jQuery(window).height(),
        documentScrollTop = jQuery(document).scrollTop(),
        documentScrollLeft = jQuery(document).scrollLeft(),

        elementOffset = jqElement.offset(),
        elementHeight = jqElement.height(),
        elementWidth = jqElement.width(),
